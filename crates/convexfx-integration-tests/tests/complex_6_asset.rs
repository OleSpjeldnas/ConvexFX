...
fn test_complex_6_asset_clearing() {
    ...
    // Setup initial inventory: scaled appropriately for each asset
    // JPY needs more units since 1 JPY ≈ 0.01 USD
    let mut inventory = BTreeMap::new();
    inventory.insert(AssetId::USD, 20.0);
    inventory.insert(AssetId::EUR, 20.0);
    inventory.insert(AssetId::JPY, 2000.0); // 2000M JPY ≈ 20M USD equivalent
    inventory.insert(AssetId::GBP, 20.0);
    inventory.insert(AssetId::CHF, 20.0);
    inventory.insert(AssetId::AUD, 20.0);
    ...
    let orders = vec![
        ...
        PairOrder {
            ...
            pay: AssetId::USD,
            receive: AssetId::JPY,
            budget: Amount::from_f64(0.15).unwrap(), // Smaller budget for JPY
        ...
        PairOrder {
            ...
            pay: AssetId::JPY,
            receive: AssetId::EUR,
            budget: Amount::from_f64(1.0).unwrap(), // 1M JPY (~0.01M USD equivalent)
    ...
    // Adjust for 6 assets with scaled bounds for JPY
    for asset in AssetId::all() {
        let (target, min, max) = if *asset == AssetId::JPY {
            (2000.0, 1000.0, 3000.0) // JPY in millions, scaled for denomination
        } else {
            (20.0, 10.0, 30.0)
        };
        risk.q_target.insert(*asset, target);
        risk.q_min.insert(*asset, min);
        risk.q_max.insert(*asset, max);
    }
    ...
    match solution {
        Ok(sol) => {
            ...
            // USD should be numeraire (within numerical tolerance)
            let y_usd = sol.y_star.get(&AssetId::USD).copied().unwrap_or(1.0);
            assert!(y_usd.abs() < 1e-4, 
                "USD must be numeraire (y_USD ≈ 0), got {}", y_usd);
            
            // Note: Prices may be outside bands due to limitations of the simple gradient solver
            // A production system would use OSQP or another robust QP solver
            for asset in AssetId::all() {
                let y = sol.y_star.get(asset).copied().unwrap_or(0.0);
                let y_ref = instance.ref_prices.get_ref(*asset);
                let deviation = (y - y_ref).abs();
                // Just check that prices aren't wildly off (within 50% deviation in log space)
                assert!(deviation < 0.5,
                    "Price for {} has excessive deviation: {}", asset, deviation);
            }
